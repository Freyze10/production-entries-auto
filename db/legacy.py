...
class Sync(QObject):
    ...
    def run(self):
        ...
        # ── FORMULA SYNC ──────────────────────────────────────────────────────
        try:
            # -- Step 0: Load customer name → customer_id lookup from tbl_customer
            self.progress.emit("Loading customer lookup table...")
            with engine.connect() as conn:
                rows = conn.execute(
                    text("SELECT customer_id, customer_name FROM tbl_customer")
                ).fetchall()

            # Keyed by normalised uppercase name for case-insensitive matching
            customer_lookup: dict[str, int] = {
                row.customer_name.strip().upper(): row.customer_id
                for row in rows
            }
            self.progress.emit(f"Loaded {len(customer_lookup)} customers into lookup.")

            # -- Step 1: Read formula items
            self.progress.emit("Phase 1/3: Reading local formula items...")
            ...
            # -- Step 2: Read formula primary records
            ...
            for r in dbf_primary:
                ...
                # Resolve customer_id from tbl_customer via customer name
                raw_customer_name = str(r.get('T_CUSTOMER', '') or '').strip()
                customer_id = customer_lookup.get(raw_customer_name.upper())

                if customer_id is None:
                    # Log unmatched names but still insert with NULL so the
                    # record is not silently dropped — adjust to `continue`
                    # if you prefer to skip unmatched records entirely.
                    unmatched_customers.add(raw_customer_name)
                    print(f"WARNING: No customer_id found for name '{raw_customer_name}' (uid={uid}). Inserting as NULL.")

                primary_recs.append({
                    "formula_index":       str(r.get('T_INDEX', '') or '').strip(),
                    "uid":                 uid,
                    "formula_date":        r.get('T_DATE'),
                    "customer_id":         customer_id,   # ← resolved from tbl_customer
                    ...
                    "mix_type":            str(r.get('T_MIX', '') or '').strip(),
                    "resin":               str(r.get('T_RESIN', '') or '').strip(),
                    ...
                    "total_concentration": _to_float(r.get('T_TOTALCON')),
                    "is_used":             bool(r.get('T_USED')),
            ...
            if unmatched_customers:
                self.progress.emit(
                    f"WARNING: {len(unmatched_customers)} unmatched customer name(s): "
                    f"{', '.join(sorted(unmatched_customers))}"
                )

            if not primary_recs:
                self.finished.emit(True, "Sync Info: No new formula records found to sync.")
                return

            all_items_to_insert = [
                item for rec in primary_recs
                for item in items_by_uid.get(rec['uid'], [])
            ]
                ...
                with conn.begin():
                    conn.execute(text("""
                        INSERT INTO tbl_formula01 (
                            form_id, index_no, date, customer_id, prod_code, prod_color,
                            dosage, total_concentration, ld, mix_time, resin, application,
                            colormatch_no, colormatch_date, notes, date_time, is_deleted, is_used
                        )
                        VALUES (
                            :uid, :formula_index, :formula_date, :customer_id, :product_code,
                            :product_color, :dosage, :total_concentration, :ld, :mix_type,
                            :resin, :application, :cm_num, :cm_date, :remarks,
                            :dbf_updated_on_text, :is_deleted, :is_used
                        )
                    ...
                    if all_items_to_insert:
                        conn.execute(text("""
                            INSERT INTO tbl_formula02 (
                                form_id, sequence_no, material_code, concentration, is_deleted
                            )
                            VALUES (
                                :uid, :seq, :material_code, :concentration, :is_deleted
                            )
                        """), all_items_to_insert)
